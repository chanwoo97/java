...
public class Inheritance_Ex1 {
    ...
    // 저그는 스타크래프트의 한 종족이다. Zerg is StarcraftRace. (성립)

    // 안되는 예)
    // 저그는 운동선수이다. Zerg is Athlete. (성립 안됨)
    ...
    // 이용해서, 자식 인스턴스를 생성할 때,
    // A a = new A();
    // Dog_Ex1 dog_Ex1 = new Dog_Ex1(); -> 1) 객체생성, 2) 인스턴스 생성했다, 3) 기본 생성자로 초기화 했다. => 셋다 같은말 
    // 초기화 할 때 뭐야? 1) 생성자 호출, 기본값으로 할당이 된다는 말.
    // 클래스 -> null, 문자열 마찬가지고, int 정수이면 0으로 초기화 된다는 말.
