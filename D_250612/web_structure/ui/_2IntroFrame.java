...
public class _2IntroFrame extends JFrame { // JFrame 부모 클래스를 상속받아서, 기능이 많아짐.
    // 실행 클래스가 아니라, 설계 클래스 이므로, 생성자에 필요한 기능을 넣고,
    // 이 클래스의 기본 생성자를 호출(인스턴스 생성시) 동작 하게끔하는 원리.

    // 실행시
    // 순서2
    public _2IntroFrame() {
        // JFrame frame = new JFrame("창의 제목 작성");
        // JFrame 상속을 받았기 때문에, 이 클래스는 임의 기능을 탑재하고 있음.

        setTitle("인트로 화면 예시"); // 창 제목
        setSize(400, 200); // 창 크기 설정
        setLocationRelativeTo(null); // 화면 중앙 배치 기능
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 창 닫기 버튼 클릭시, 프로그램 종료

        // 안내 문구 표시
        JLabel label = new JLabel("환영합니다. 3초 후 메인으로 이동합니다.", JLabel.CENTER);
        // 프레임에 붙이기
        add(label);
        // 화면에 보이기
        setVisible(true);
